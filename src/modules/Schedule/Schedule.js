...
class Schedule extends Component {
    ...
    componentWillMount(){
        db.child("schedule").child(this.props.match.params.grado).child(this.props.match.params.grupo).orderByChild("CC")
        .once("value", schedule => {
            let classArray = [];
            schedule.forEach(function (clase){
                classArray.push(clase.val());
            });
            if(classArray.length>0) toastr.success("Horario cargado con exito");
